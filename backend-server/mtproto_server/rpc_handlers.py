...
def _skip_json_value(reader):
    """Skip a serialized JSONValue TL object in the reader."""
    cid = reader.read_uint32()
    if cid == 0x3f6d7b68:    # jsonNull
        pass
    elif cid == 0xc7345e6a:  # jsonBool
        reader.read_uint32()  # boolTrue/boolFalse
    elif cid == 0x2be0dfa4:  # jsonNumber
        reader.read_raw(8)    # double
    elif cid == 0xb71e767a:  # jsonString
        reader.read_string()
    elif cid == 0xf7444763:  # jsonArray
        reader.read_uint32()  # vector constructor
        count = reader.read_int32()
        for _ in range(count):
            _skip_json_value(reader)
    elif cid == 0x99c1d49d:  # jsonObject
        reader.read_uint32()  # vector constructor
        count = reader.read_int32()
        for _ in range(count):
            reader.read_uint32()  # jsonObjectValue constructor
...
def unwrap_layers(data: bytes, ctx: RPCContext) -> tuple:
    ...
    if constructor == C.INIT_CONNECTION:
        ...
        if flags & 1:
            ...
            reader.read_int32()   # port
        if flags & 2:
            # JSONValue — skip the TL object
            _skip_json_value(reader)
...
HANDLERS = {
    ...
    C.LANGPACK_GET_LANGUAGES_2: handle_langpack_get_languages,
    C.LANGPACK_GET_LANG_PACK: handle_langpack_get_lang_pack,
    C.LANGPACK_GET_STRINGS: handle_langpack_get_strings,
    C.LANGPACK_GET_STRINGS_2: handle_langpack_get_strings,
